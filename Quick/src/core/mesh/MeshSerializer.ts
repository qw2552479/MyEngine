namespace QE {
    ...
    export class MeshSerializer {
        public static serializeByJson(meshData: MeshData, mesh: Mesh) {
            ...
            for (let i = 0, len = subMeshes.length; i < len; i++) {
                ...
                for (let j = 0; j < indicesData.length; j++) {
                ...
                // 子网格索引数据
                const indicesBuf = QE.WebGLBufferManager.instance.createIndexBuffer(indices, QE.BufferUsage.STATIC, false);
